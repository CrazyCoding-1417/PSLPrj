...
// Change
  ...
  const [isCompMounted, setIsCompMounted] = useState(true)
...
// Change end
  ...
  async fetchProductUsingID(id) {
    ...
    setUnfinishedTasks(ps.unfinishedTasks + 1)
    ...
    if (this.isCompMounted) {
      ...
      // });
      setUnfinishedTasks(ps.unfinishedTasks - 1)
      setRelatedItems(relatedItems.data.filter(x => x.id !== item.id))
    }
  }
  ...
  //   this.fetchProductUsingID(props.match.params.id);
  // }
  //
  ...
  useEffect(()=>{
    fetchProductUsingID(props.match.params.id)
  }, [])
